...
The shared study navigation is added during the build, so regenerated reading archives keep the Revisions link without rewriting article content. Run `npm run build` before `npm run dev`. The test covers displayed app rows001–037, including four separately scored gaps, not37 checkable video entries. Browser quizzes, automatic grading and progress marking are intentionally not part of this handwritten revision.
