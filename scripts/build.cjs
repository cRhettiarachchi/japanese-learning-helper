...
for(const name of [...files.filter(name=>name.endsWith('.html')),'revisions.html','revisions/revision-1-test.html','revisions/revision-1-answers.html']){
 ...
 let enhanced=html.replace(/<nav\b[^>]*class="(?:site-nav|study-nav)"[^>]*>[\s\S]*?<\/nav>/g,nav=>nav.includes('href="/vocabulary.html"')?nav:nav.replace('</nav>','<a href="/vocabulary.html">Vocabulary</a></nav>'));
 if(enhanced.includes('src="/dictionary.js"'))enhanced=enhanced.replace('<script src="/dictionary.js"','<script src="/vocabulary-client.js" defer></script><script src="/dictionary.js"');
 fs.writeFileSync(file,enhanced.replace('</head>','<link rel="stylesheet" href="/study-timer.css"><script src="/study-timer.js" defer></script></head>'));
}
