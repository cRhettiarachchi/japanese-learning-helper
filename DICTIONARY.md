...
`dictionary.js` and `dictionary.css` provide one non-modal popup for articles and transcripts. It lists all English glosses from the matched local JMdict entries, deduplicating repeated gloss text within each entry without limiting or hiding senses. Vocabulary review adds an account-backed Add to review / Added button. When multiple entries match, canonical headwords separate their meanings so the learner explicitly chooses which entry to save. Pronunciation and part-of-speech labels remain omitted from the lookup popup. See VOCABULARY.md for the review schedule and persistence. The selected word remains highlighted on the page. Close/Escape returns focus to the selected word. Loading, Retry and no-entry states remain available. Dictionary attribution/licensing is retained in a compact footer.
