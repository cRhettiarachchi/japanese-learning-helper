...
`src/components/dictionary-panel.tsx` provides one accessible shadcn sheet for articles and transcripts, with a full-width bottom panel on phones. It lists all English glosses from the matched local JMdict entries, deduplicating repeated gloss text within each entry without limiting or hiding senses. Vocabulary review adds an account-backed Add to review / Added button. When multiple entries match, canonical headwords separate their meanings so the learner explicitly chooses which entry to save. Canonical Japanese readings accompany each headword; part-of-speech labels remain omitted. See VOCABULARY.md for the review schedule and persistence. The selected word remains highlighted on the page. Close/Escape returns focus to the selected word. Vocabulary saving failures provide Retry; unmatched tokens retain a no-entry state. Dictionary attribution/licensing is retained in a compact footer.

The content compiler includes the exact local dictionary entries referenced by each document. React word buttons and transcript controls share the sheet; full/focused views, word/timestamp highlighting, speed/readings controls, and pause-on-lookup/resume-only-if-previously-playing behavior are preserved. There are no external per-word lookup requests.
...
The normal `npm run build` compiles the committed HTML content sources and local dictionary data into React routes; Vercel does not need Python or the full dictionary. To regenerate word buttons after editing/adding articles, install `scripts/requirements-dictionary.txt` in a Python environment, obtain JMdict English from EDRDG, and run:
